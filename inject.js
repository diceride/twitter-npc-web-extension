const npcAvatarSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 181.42 181.42">
...
function checkAnnoyances(imgElements) {
  return Array.from(imgElements).some((imgElement, index) => {
    if (imgElement.alt) {
      if (imgElement.alt === '🇺🇦' || (imgElement.alt === '💙' && imgElements[index + 1].alt === '💛')) {
        return true;
      }

      if (imgElement.alt.includes('🌈') || imgElement.alt.includes('⚧')) {
        return true;
      }

      return ['😷', '💉', '🇪🇺', '🇺🇳'].includes(imgElement.alt);
    }
...
function replaceAvatar(imgElement) {
  let npcAvatarFragment = document.createRange().createContextualFragment(npcAvatarSvg);

  imgElement.parentElement.firstChild.replaceWith(npcAvatarFragment.firstChild);
  imgElement.remove();
}

function scanDOM() {
  // Only present when on a profile page
  const userNameElement = document.querySelector('[data-testid="UserName"]');
  if (userNameElement) {
    // Query all img elements in the user's name
    const emojiElements = userNameElement.querySelectorAll('img');
    if (emojiElements.length > 0 && checkAnnoyances(emojiElements)) {
      // Query the user's avatar element
      const avatarElement = userNameElement.parentElement.firstElementChild.querySelector('img');
      if (avatarElement) {
        replaceAvatar(avatarElement);

        try {
          // Notify the chrome service worker
  ...
  if (articleElements.length > 0) {
    articleElements.forEach((articleElement) => {
      // Query all a anchor elements of the tweet. The first a anchor element contains
      // the user's avatar img, the second a anchor element contains the user's name
      const aElements = articleElement.querySelectorAll('a[role="link"]');
      if (aElements.length >= 2) {
        // Query all img elements in the user's name
        const emojiElements = aElements[1].querySelectorAll('img');
        if (emojiElements.length > 0 && checkAnnoyances(emojiElements)) {
          // Query the user's avatar element
          const avatarElement = aElements[0].querySelector('img');
          if (avatarElement) {
            replaceAvatar(avatarElement);
  ...
  // Only present when an overlay is activated
  const aElements = document.querySelectorAll('#layers a[role="link"]');
  if (aElements.length >= 2) {
    // Query all img elements in the user's name
    const emojiElements = aElements[1].querySelectorAll('img');
    if (emojiElements.length > 0 && checkAnnoyances(emojiElements)) {
